...
2. Call `flow-check-profile-consistency`; receive discrepancy list, consistency score, and missing-snapshot warnings
3. For each 🔴 discrepancy: call `task-flag-profile-inconsistency` to write to open-loops
4. For each 🟡 discrepancy: call `task-flag-profile-inconsistency` with 🟡 priority
...
10. Call `task-update-open-loops` with all flags from the audit
