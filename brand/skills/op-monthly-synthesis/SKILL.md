...
Calls `flow-build-analytics-summary` to compile cross-platform metrics with MoM comparisons. Calls `flow-analyze-mentions` to assess sentiment distribution and identify notable mentions. Evaluates four scoring dimensions and combines them into the 0-100 health score.
...
2. Call `flow-build-analytics-summary` for cross-platform metrics and top content
3. Call `flow-analyze-mentions` for sentiment distribution and notable mentions
4. Call `flow-check-profile-consistency` (lightweight — for consistency score component only)
...
12. Call `task-update-open-loops` with all flags from this synthesis
