...
- "reconnect with app-contacts"

## Steps

1. Read network contact log from `vault/career/` — load all app-contacts with last interaction date, relationship type, and strategic notes.
...
4. Score warm and cooling app-contacts by strategic priority: target company hiring managers (highest), former colleagues in relevant roles, connectors in your industry, mentors/sponsors.
5. Select top 3-5 app-contacts for outreach this month from warm and cooling groups, weighted by strategic priority.
...
9. Write network review summary to `vault/career/02_briefs/YYYY-MM-network-review.md` with selected app-contacts, draft messages, and rationale.
...
Active (last 60 days): X app-contacts
Warm (60-90 days): X app-contacts — [names of top priority]
Cooling (90-180 days): X app-contacts
Dormant (180+ days): X app-contacts — flagged for decision
...
## Pipeline Follow-Ups (via network app-contacts)
...
- `app-linkedin.portal_profile_url` — your profile for new connection scanning
- `app-linkedin.portal_chrome_profile` — path to Chrome profile
...
- **Contact log empty or missing:** Prompt user to populate `vault/career/` with at least their top 10-20 strategic app-contacts before the network review can produce useful output.
- **LinkedIn access unavailable:** Skip new connection scan; complete the rest of the review from vault contact log alone.
- **No app-contacts in warm/cooling range:** Report that all app-contacts are either active or dormant. For dormant app-contacts, present the list and ask the user to identify which are worth attempting to revive with a specific reconnect hook.
