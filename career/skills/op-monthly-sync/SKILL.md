...
cadence: monthly
description: >
  Full career data sync on the 1st of each month. Refreshes compensation records from payroll app-insurance-portal.portal, updates the job application pipeline, syncs LinkedIn profile activity and recruiter messages, organizes career documents into vault, and triggers the monthly career review brief. Triggers: "career monthly sync", "sync career data", "refresh career vault", "update career vault".
---
...
**Payroll sync:** Pulls the most recent pay stub from the configured payroll app-insurance-portal.portal (ADP Workforce Now or Workday) via Playwright and extracts: gross pay, net pay, YTD earnings, current 401k deduction and YTD contributions, and all benefit deductions. Saves to `vault/career/00_current/pay-stubs/`. If the most recent pay stub shows a compensation change (raise, bonus payout, or deduction change), this is flagged for the Chief of Staff to route to the Wealth plugin.

**Pipeline sync:** Calls `flow-review-pipeline` to audit all active pipeline items for staleness. Applications with no response in 7+ business days are flagged for follow-up. Opportunities at the same stage for 14+ days without a next step are marked stalled. Watch-list items from prior month's market scan that have since closed are archived.

**LinkedIn activity sync:** Checks LinkedIn profile views and search appearance statistics (visible in LinkedIn Premium or via profile analytics) and logs them to `vault/career/00_current/linkedin-activity.md`. Scans the LinkedIn inbox for any pending recruiter messages or connection requests from people at target companies and logs them as pipeline items or network app-contacts.
...
Ends by triggering `op-review-brief` to produce the monthly brief with the freshly synced data.
...
1. Read `vault/career/config.md` — confirm payroll app-insurance-portal.portal type (ADP or Workday), LinkedIn profile URL, and Chrome profile path.
2. Connect to payroll app-insurance-portal.portal via Playwright (headless=False) — navigate to pay statements, download most recent pay stub PDF to `vault/career/00_current/pay-stubs/YYYY-MM-paystub.pdf`.
...
5. Call `flow-review-pipeline` — get pipeline status report with stale and stalled flags.
...
12. Call `op-review-brief` to produce the monthly brief with refreshed data.
13. Call `task-update-open-loops` with all flags from this sync run.

## Input

- `~/Documents/aireadylife/vault/career/config.md` — app-insurance-portal.portal URLs, Chrome profile, LinkedIn handle
- ADP / Workday payroll app-insurance-portal.portal (via Playwright)
...
- `payroll_app-insurance-portal.portal` — app-adp or app-workday
- `payroll_app-insurance-portal.portal_url` — specific URL for your employer's app-insurance-portal.portal
- `payroll_chrome_profile` — path to Chrome profile with saved session
- `app-linkedin.portal_profile_url` — your LinkedIn profile URL
- `app-linkedin.portal_chrome_profile` — path to Chrome profile with saved LinkedIn session

## Error Handling

- **Payroll app-insurance-portal.portal login expired:** Note that pay stub sync failed, prompt user to re-authenticate via Chrome (headless=False), then retry. Do not block the rest of the sync.
