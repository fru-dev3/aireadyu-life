...
11. Call `flow-build-coverage-summary` for structured coverage table.
12. Write coverage audit to `vault/benefits/02_briefs/coverage-audit-QN-YYYY.md` with per-line status and gap analysis.
13. Call `task-update-open-loops` with all identified gaps and any administrative discrepancies.
