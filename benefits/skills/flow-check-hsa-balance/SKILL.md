...
Called by `op-hsa-review` to produce the core HSA analysis data. Reads all HSA account data from `vault/benefits/00_current/` and performs all calculations needed for the monthly HSA review. Returns structured data to the calling op for brief writing and open loop flagging.
