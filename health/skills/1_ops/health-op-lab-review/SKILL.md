...
Runs whenever new lab results arrive from a patient portal or uploaded manually. It reads the incoming PDF or structured data, parses each biomarker value, and compares it against standard reference ranges. Any value outside the normal range is handed off to `arlive-health-flag-out-of-range-value` for logging — the flag records the biomarker name and severity but deliberately omits raw values to avoid storing PHI in plain text. The op then calls `arlive-health-build-lab-summary` to produce a consolidated panel view grouped by test category (metabolic, lipid, CBC, thyroid, etc.) with trend arrows vs. the prior panel.

## Calls

- **Flows:** `arlive-health-build-lab-summary`
- **Tasks:** `arlive-health-flag-out-of-range-value`, `arlive-health-update-open-loops`
