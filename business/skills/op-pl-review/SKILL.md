...
Calls `task-flag-overdue-invoice` to scan the invoice file for any unpaid invoices where the due date has passed 30 days or more. Calls `flow-build-pl-summary` to produce the formatted P&L table. Writes the complete dated brief to `vault/business/02_briefs/pl-{YYYY-MM}.md` and pushes all action items to `vault/business/open-loops.md`.
...
5. Call `flow-build-pl-summary` to calculate and format the P&L table with MoM comparison
...
8. Call `task-flag-overdue-invoice` to scan for unpaid invoices >30 days past due
...
12. Call `task-update-open-loops` with all flags
