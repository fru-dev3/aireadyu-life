...
Called by `op-monthly-sync` and `op-progress-review` to produce the core learning pace analysis. This flow handles the quantitative calculation layer — reading all active items, computing pace metrics, and producing a structured table — while the calling op handles brief writing and flag routing.
