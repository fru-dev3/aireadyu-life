...
**Platform progress refresh:** Connects to each configured learning platform (Coursera, LinkedIn Learning, Udemy, Pluralsight, A Cloud Guru, O'Reilly, Educative) via Playwright and pulls current completion status for each enrolled course. For each course: total modules or hours, completed modules or hours, completion percentage, last activity date, and any assignment or quiz deadlines. Saves updated progress to `vault/learning/01_courses/` with a sync timestamp. Calculates whether each course is ahead of or behind its target completion pace using: (completion %) vs. (days elapsed since start ÷ total days to target completion date).

**Reading list sync:** Updates the reading list status in `vault/learning/02_books/` based on Goodreads RSS (if synced) or manual progress entries. Records: current book title, percentage complete (pages read ÷ total pages), books completed YTD (count and titles), and current reading pace (books/month). Compares YTD completion to annual reading goal pace.
...
**Monthly goal vs. actual review:** Reads the monthly learning targets from `vault/learning/03_goals/` (how many hours, which milestones, what completion targets) and compares against actual progress logged in the sync. Calculates the achievement rate (actual hours ÷ target hours, milestones hit ÷ milestones planned).
...
3. Update `vault/learning/01_courses/` with current progress data for each course, noting sync timestamp.
4. Check Goodreads RSS or `vault/learning/02_books/current-reading.md` for reading progress — update current book percentage and add any recently completed books.
...
7. Read monthly milestone targets from `vault/learning/03_goals/` — compare to actual progress.
...
- Goodreads RSS or `~/Documents/AIReadyLife/vault/learning/02_books/current-reading.md`
...
Brief: Generated at vault/learning/03_briefs/[file]
```
...
- **Goodreads RSS not configured:** Fall back to reading progress from `vault/learning/02_books/current-reading.md` (manual updates).
- **No certification goals configured:** Skip certification timeline section with note.

## Vault Paths

- Reads from: `~/Documents/AIReadyLife/vault/learning/config.md`, `~/Documents/AIReadyLife/vault/learning/00_current/certs.md`, `~/Documents/AIReadyLife/vault/learning/03_goals/`
- Writes to: `~/Documents/AIReadyLife/vault/learning/01_courses/`, `~/Documents/AIReadyLife/vault/learning/02_books/`, `~/Documents/AIReadyLife/vault/learning/00_current/status.md`, `~/Documents/AIReadyLife/vault/learning/open-loops.md`
