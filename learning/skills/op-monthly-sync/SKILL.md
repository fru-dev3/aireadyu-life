...
Ends by triggering `op-review-brief` with the freshly synced data to produce the weekly brief for the new month.
...
8. Identify all active learning items where completion % is behind time-elapsed %: calculate the deficit and flag to `task-flag-falling-behind`.
9. Update `vault/learning/00_current/status.md` with sync timestamp and summary statistics.
10. Call `op-review-brief` to produce the monthly brief.
11. Call `task-update-open-loops` with all flags from this sync.
