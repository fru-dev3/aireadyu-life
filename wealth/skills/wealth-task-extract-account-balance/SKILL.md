...
description: >
  ...
  name from vault/wealth/00_accounts/ and returns the structured record to the calling
...
The "prior period balance" is the balance recorded at the end of the previous month — used by calling flows for MoM delta calculations. The task reads from the account's structured file in `vault/wealth/00_accounts/[account-nickname].md` which is maintained in a consistent format across all accounts.
