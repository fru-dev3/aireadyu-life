...
After presenting the topic brief, asks whether to: (1) save the brief to vault/intel/01_briefs/ as a one-time reference, (2) create a new thread in vault/intel/02_threads/ to track this topic across future daily briefings, or (3) both. If the user creates a thread, writes a thread file with today's brief as the opening entry and a thread status of "developing." Future daily briefings will then append updates to this thread automatically.
...
2. Check `~/Documents/AIReadyLife/vault/intel/02_threads/` for an existing thread on this topic; if found, offer to update the existing thread rather than creating a new brief
...
6. If user says yes to thread tracking: write thread file to vault/intel/02_threads/{topic-slug}.md with today's brief as the opening entry and status: developing
...
- `~/Documents/AIReadyLife/vault/intel/02_threads/` — for existing thread check
- `~/Documents/AIReadyLife/vault/intel/01_briefs/` — for prior coverage context
- `~/Documents/AIReadyLife/vault/intel/00_sources/` — source registry for credibility reference
...
- If an existing thread is found: "You already have an open thread on this topic from {date}. View or update it at vault/intel/02_threads/{topic-slug}.md?"
...
- Writes to: `~/Documents/AIReadyLife/vault/intel/02_threads/{topic-slug}.md` (if thread created), `~/Documents/AIReadyLife/vault/intel/01_briefs/{YYYY-MM-DD}-{topic-slug}-deep-dive.md` (if saved as brief)
