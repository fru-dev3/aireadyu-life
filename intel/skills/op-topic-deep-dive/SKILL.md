...
Calls `flow-build-topic-summary` to do the analytical synthesis work. The deep dive op adds the user-facing experience layer: clarifying the topic scope if it is ambiguous, asking whether to track this as an ongoing thread in the vault, and providing the formatted output with clear sections.
...
3. Call `flow-build-topic-summary` with the topic name and any focus angle specified
...
8. Call `task-update-open-loops` if any priority story or watch signal emerged from the deep dive
