...
else. Writes the dated brief to vault/ben/00_briefs/ and calls flag-urgent-item for any 🔴
items that need to be preserved in vault/ben/01_alerts/ for tracking.

## Configuration

No special configuration required beyond having plugins installed with their vault/ subfolders
present. Optionally maintain vault/ben/config.md with a list of plugin names to scan, or the
...
`vault/ben/00_briefs/daily-{date}.md`
