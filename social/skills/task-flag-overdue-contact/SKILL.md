...
**Resolution:** The flag is marked resolved when a new interaction is logged in vault/social/00_current/ for that contact. The next time `task-update-open-loops` runs, it detects the new interaction and marks the corresponding open-loop flag as complete.
