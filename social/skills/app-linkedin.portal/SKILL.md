...
for professional-tier app-contacts. LinkedIn is used for reconnection context (what someone is working
...
- Work anniversary notifications — reconnect opportunities for professional app-contacts
- Mutual connections count — useful when warming up a dormant professional relationship
- "People You May Know" — optional; identifies new app-contacts to add to vault
...
app-linkedin.portal_connections_export_path: ~/Documents/aireadylife/vault/social/00_current/linkedin-connections.csv
```
...
- LinkedIn connections are not the same as real relationships. Use LinkedIn data as context for
  professional-tier app-contacts (T3/T4) already in the vault — do not auto-create vault records for
  every LinkedIn connection.
- Work anniversaries and job changes are high-quality reconnect triggers for professional app-contacts
...
- `op-monthly-sync` — check for new connections added since last sync; identify LinkedIn
  app-contacts who should be added to vault contact files; pull recent activity for context refresh
- `flow-build-outreach-queue` — enrich queue entries for professional app-contacts with current
