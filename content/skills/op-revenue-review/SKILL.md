...
Calls `flow-build-revenue-summary` to aggregate YouTube AdSense earnings, newsletter revenue (sponsorship + paid subscriptions as separate line items), and Gumroad digital product revenue by individual product. Calculates MoM change per channel and total. Identifies the top revenue channel and flags any channel with >20% MoM decline.
...
10. Call `task-update-open-loops` with revenue decline flags and anomalies
