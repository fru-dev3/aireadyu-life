...
Records individual revenue events to the vault so `flow-build-revenue-summary` has clean, structured data to aggregate from. The log is the single source of truth for all content revenue — if it is not logged here, it will not appear in monthly revenue reviews, YTD calculations, or revenue trend analysis.
...
- "log a product sale"
- Called by `op-revenue-review` after each monthly review cycle
