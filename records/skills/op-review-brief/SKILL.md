...
cadence: monthly
description: >
  Monthly records review brief. Compiles expiring documents (with renewal steps), the full
  subscription review (total spend, unused services, upcoming renewals, keep/cancel
  recommendations), upcoming renewals across categories, document gaps, legal document
  review flags, and storage gaps into a single briefing with action items sorted by urgency.
  Subsumes the prior standalone subscription-review op — there is no separate subscription
  brief; the table lives here.
  Triggers: "records brief", "document review", "subscription audit", "what's expiring",
  "what am I paying for", "subscription review".
---
...
This brief includes the full **subscription review** (formerly its own op): every active subscription with monthly + annual cost, last-used date, usage tier (keep / review / cancel), renewals due within 30 days, category duplicates, and total recurring spend. The subscription registry itself is built by `task-detect-subscriptions-from-email`; this op consumes that table directly.
...
The subscription cost section shows the full subscription table read directly from `00_current/subscriptions.md` (rebuilt by `task-detect-subscriptions-from-email`): every active subscription, monthly cost, last used, and keep/review/cancel recommendation. The section is headed with the total monthly and annual recurring spend so the number is visible even to a reader who skims. Subscriptions with renewals in the next 30 days are pulled to the top of the table with the renewal date bolded. Unused subscriptions (no use in 60+ days) are grouped at the bottom with the estimated annual savings from canceling all of them. Category duplicates (two cloud-storage services, two music streamers) are surfaced as a consolidation prompt.
...
3. Read subscription registry from `~/Documents/aireadylife/vault/records/00_current/subscriptions.md` (rebuild via `task-detect-subscriptions-from-email` if stale)
3a. Read renewal calendar from `~/Documents/aireadylife/vault/records/00_current/renewal-calendar.md` (rebuild via `flow-renewal-calendar` if stale)
...
- `~/Documents/aireadylife/vault/records/00_current/renewal-calendar.md`
