...
2. Call `flow-check-travel-docs` for full document inventory validation
3. For each document expiring within warning thresholds: call `task-flag-expiring-document`
4. Read vault/explore/00_current/ for any trips with departure within 90 days
5. For each upcoming trip: call `flow-build-trip-summary` to get booking status
...
9. Call `task-update-open-loops` with all new flags
