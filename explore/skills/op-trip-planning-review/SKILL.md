...
3. If no trip record exists: run `task-log-trip` to create one before proceeding
4. Call `flow-check-travel-docs` with trip destination, dates, and all traveler passports
5. Flag any document issue that would prevent travel as 🔴 critical
6. Call `flow-build-trip-summary` for booking status and budget summary
...
9. Call `task-update-open-loops` to write preparation items to vault/explore/open-loops.md
...
- **No trip record found:** Ask user for trip details (destination, dates, travelers) then call task-log-trip to create the record before proceeding.
