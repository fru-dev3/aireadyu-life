...
This flow reads the full travel document inventory from vault/explore/01_documents/ and validates each document against two sets of requirements: the absolute expiry thresholds (the document's calendar expiration date vs. today) and the trip-specific requirements (the document's validity vs. upcoming trip dates with applicable rules applied).

**Document inventory reading:** The flow reads all files in vault/explore/01_documents/: passport.md (one per traveler), global-entry.md, tsa-precheck.md, nexus.md if applicable, visa files in vault/explore/01_documents/visas/, and vaccination records in vault/explore/01_documents/vaccinations/. For each document, it reads the key fields: document type, person name, expiration date, issuing country, and document number.
...
1. Read all files in vault/explore/01_documents/ (passports, Global Entry, TSA PreCheck, visas, vaccinations)
...
6. Check whether applicable visa exists in vault/explore/01_documents/visas/
7. Check vaccination requirements for destination; verify against vault/explore/01_documents/vaccinations/
...
- **Document file missing for a traveler:** Return "No [document type] record found for [traveler] — add to vault/explore/01_documents/." Flag as status unknown (🟡 by default).
...
- Reads from: ~/Documents/AIReadyLife/vault/explore/01_documents/, ~/Documents/AIReadyLife/vault/explore/config.md
