...
**Resolution condition:** The flag is marked resolved when the task is called with a completion signal (the renewal was submitted and the new document is on file), or when `task-update-open-loops` detects that the document record in vault/explore/00_current/ has been updated with a new expiry date beyond the alert window.
