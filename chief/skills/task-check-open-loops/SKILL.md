...
**Trigger:** Called by `op-weekly-preview` and any flow needing a current backlog snapshot
...
This task intentionally does not return item text — that is the job of `flow-collect-domain-alerts`. check-open-loops is a count-only operation designed to be fast and to give the calling flow a week-over-week trend signal without having to parse and re-sort the full alert list.
