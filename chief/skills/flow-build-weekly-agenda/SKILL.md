...
**Trigger:** Called by `op-weekly-preview`
...
**Backlog summary:** Renders the backlog count data provided by task-check-open-loops as a formatted table (Domain | Total | 🔴 | 🟡 | 🟢). Adds a note if any domain has 5+ unresolved items (potential backlog accumulation) or if the total count has grown compared to the prior week's brief.
...
- Sorted domain alerts list with due dates (from flow-collect-domain-alerts, via calling op)
...
- Backlog counts by domain and tier (from task-check-open-loops, via calling op)
...
| benefits | 2     | 0  | 1  | 1  |
| TOTAL    | 5     | 1  | 1  | 3  |
[Note: benefits has grown 3 consecutive weeks — consider running op-review-brief]
```
