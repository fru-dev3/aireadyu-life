...
2. Call `flow-collect-domain-alerts` to scan all vault/*/open-loops.md files
3. Parse all unresolved items; extract priority, description, action, due date, date raised per item
4. Call `task-pull-domain-status` for each discovered domain; collect last-updated, score, open-loop count
