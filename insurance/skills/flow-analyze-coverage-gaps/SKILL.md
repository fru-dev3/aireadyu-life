...
Called by `op-coverage-audit` to perform the quantitative gap analysis. Reads coverage limits from `vault/insurance/00_current/` and applies domain-specific adequacy rules to produce a scored, ranked gap list with financial exposure quantification and estimated cost to resolve.
