...
2. **New claim:** Identify policy type and read relevant policy from `vault/insurance/01_policies/`. Verify incident is covered. Provide documentation checklist. Give carrier claims contact info. Create claims log entry in `vault/insurance/02_claims/`.
3. **Active claims:** Read all open claims from `vault/insurance/02_claims/`. For each: calculate days since last activity. Flag claims with no update in 14+ days as stalled. Generate adjuster follow-up action items.
...
7. Write claims review summary to `vault/insurance/02_claims/claims-review-YYYY-MM-DD.md`.
8. Call `aireadylife-insurance-task-update-open-loops` with all claim action items and deadlines.

## Input

- `~/Documents/AIReadyLife/vault/insurance/02_claims/` — active and recent claims log
- `~/Documents/AIReadyLife/vault/insurance/01_policies/` — policy documents for coverage verification
- `~/Documents/AIReadyLife/vault/insurance/config.md` — carrier contact information

## Output Format

**Claims Review** — saved as `vault/insurance/02_claims/claims-review-YYYY-MM-DD.md`
...
Claim log entries at `vault/insurance/02_claims/` with fields: claim_number, policy_type, carrier, date_filed, adjuster_name, adjuster_phone, adjuster_email, stage, last_activity_date, settlement_offer_amount, documented_loss_amount, status (open/closed/disputed).
...
- Reads from: `~/Documents/AIReadyLife/vault/insurance/02_claims/`, `~/Documents/AIReadyLife/vault/insurance/01_policies/`, `~/Documents/AIReadyLife/vault/insurance/config.md`
- Writes to: `~/Documents/AIReadyLife/vault/insurance/02_claims/`, `~/Documents/AIReadyLife/vault/insurance/open-loops.md`
