...
Called by `op-renewal-watch` to perform the renewal date scan and action categorization. This flow handles the data layer — reading dates, categorizing renewals, and generating specific action steps — while the op handles brief writing and open loop flagging.
