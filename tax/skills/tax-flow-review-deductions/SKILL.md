...
description: >
  Scans transaction records and vault documents in vault/tax/03_deductions/ for
...
**Produces:** Updated deduction totals in `vault/tax/03_deductions/YYYY-deductions.md`

## What It Does

Reads all transaction records and uploaded receipts from `vault/tax/03_deductions/` for the current tax year and classifies each item against the applicable IRS deduction categories. This flow is the intelligence layer of deduction capture — it determines what's deductible, at what amount, and whether it's properly documented.
...
**Vehicle and Mileage.** Two methods: Standard Mileage Rate (2025 rate: 70.0 cents/mile for business, 14 cents/mile for charity, 21 cents/mile for medical) or Actual Cost (proportionate vehicle expenses). Reads the mileage log from `vault/tax/03_deductions/mileage-log.md` and computes the deductible mileage amount YTD. Flags if the mileage log has gaps (days without entries are common and do not disqualify the deduction, but contemporaneous records are required — a reconstructed log is weaker).
...
**Charitable Contributions.** Reads charitable contribution records from `vault/tax/03_deductions/charitable.md`. Cash donations require a bank record or written acknowledgment for any amount; donations ≥$250 require a written acknowledgment from the organization. Non-cash donations require a receipt with fair market value; non-cash donations >$500 require Form 8283. The flow checks that each cash donation ≥$250 has an acknowledgment reference and flags those missing it as "Documentation incomplete."
...
**YTD pace comparison.** Each deduction category's YTD total is compared to the same-period total from the prior year (read from `vault/tax/03_deductions/YYYY-1-deductions.md`). Any category running more than 20% behind is flagged as potentially underreported — either genuinely fewer expenses this year or capturing is lagging.
...
1. Read all transaction records and documents from `vault/tax/03_deductions/YYYY/`
...
4. Read mileage log from `vault/tax/03_deductions/mileage-log.md` and compute 2025 mileage deduction
...
9. Read prior year same-period totals from `vault/tax/03_deductions/YYYY-1-deductions.md` for pace comparison
10. Flag any category more than 20% behind prior year pace; write updated deduction summary to vault

## Input

- `vault/tax/03_deductions/YYYY/` — transaction records and receipts for current year
- `vault/tax/03_deductions/mileage-log.md` — mileage records for vehicle deduction
- `vault/tax/03_deductions/charitable.md` — charitable contribution records
- `vault/tax/03_deductions/YYYY-1-deductions.md` — prior year deduction totals for pace comparison
- `vault/tax/config.md` — home office sq ft, deduction methods, estimated AGI

## Output Format

Markdown document at `vault/tax/03_deductions/YYYY-deductions.md`:
...
- If mileage log is missing: flag "No mileage log found — if claiming vehicle deduction, create vault/tax/03_deductions/mileage-log.md with daily records"
...
- Reads from: `~/Documents/AIReadyLife/vault/tax/03_deductions/` (all deduction records)
- Reads from: `~/Documents/AIReadyLife/vault/tax/config.md`
- Writes to: `~/Documents/AIReadyLife/vault/tax/03_deductions/YYYY-deductions.md`
