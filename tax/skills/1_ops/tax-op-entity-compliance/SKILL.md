...
Runs quarterly to ensure all active business entities remain in good standing without surprises at filing time. It calls `arlive-tax-document-completeness` to verify that all entity-level tax obligations are tracked: annual report filings with the state, franchise tax payments, registered agent renewal dates, S-Corp election maintenance (reasonable salary, payroll tax deposits), and any state-specific LLC taxes. Each entity is checked against its jurisdiction's upcoming deadlines for the next 90 days. Missing filings, approaching deadlines, or documentation gaps are flagged via `arlive-tax-update-open-loops`. The quarterly cadence catches issues that appear only a few times per year — like annual report windows — before they become late fees or administrative dissolution risks.
