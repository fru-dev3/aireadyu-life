...
3. If no recent scorecard: call `flow-build-scorecard` to generate one now
4. Identify 2 highest-momentum domains (8+ score or largest positive trend) and 2 lowest (below 5 or largest negative trend)
5. Call `flow-score-domain-progress` to evaluate current-quarter OKR key results
...
- **No current-month scorecard and no prior scorecard:** Generate scorecard on-demand via flow-build-scorecard; note "Scorecard generated on-demand — run op-monthly-scorecard at month end for the full report."
- **No OKRs for current quarter:** Skip at-risk OKR section; note "No OKRs set for this quarter — run op-quarterly-planning to set goals."
