...
6. **Phase 2:** Call `flow-build-scorecard` for final quarterly domain health picture
7. Call `flow-score-domain-progress` for final OKR completion assessment
8. Identify top 2-3 momentum domains and top 2-3 needs-attention domains
9. **Phase 3:** Call `flow-draft-quarterly-plan` with: vision doc, domain scores, lessons learned, carry-forward milestones
...
14. Call `task-update-open-loops` to clear prior-quarter flags and write new quarter's priority flags
