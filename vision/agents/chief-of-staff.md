...
**At-risk goal escalation:** When the op-monthly-scorecard identifies a key result that is critically at-risk (less than 50% complete with less than 2 weeks remaining in the quarter), the Chief of Staff writes a 🔴 flag to vault/vision/open-loops.md so it surfaces in the Chief plugin's morning brief. The flag includes the OKR name, the KR description, the current completion percentage, the target, and the specific blocker or diagnosis. The user sees this in their morning brief under the vision domain row — not buried in the vision vault.
