...
Maintains the canonical open-loops file for the calendar domain. When called, it appends any new flags generated during the current op run — deadline clusters approaching, weeks with projected focus time deficits, high-priority items without a calendar slot, or unresolved agenda items carried over from a prior week. Each flag includes the item description, urgency level, due date if applicable, and the recommended action. Also scans existing open loop items and marks any as resolved if the underlying condition is no longer true (deadline passed, focus time target met for the week, item was scheduled). Keeps the file clean and actionable rather than accumulating stale entries. The file is read by `aireadylife-calendar-collect-deadlines` when scanning cross-domain deadlines, so accurate maintenance here directly improves every agenda build.
